...
Solução usando `str_sub()`:

```{r}
url <- c('passagens+de+aviao+exterior+&oq=viagens/1VzieO3FdTgsKvZ/calendario/hoje/')
string1 <- str_sub(url, start = 1, end = 9)
string2 <- str_sub(url, start = 11, end=12)
string3 <- str_sub(url, start = 14, end=18)
string4 <- str_sub(url, start= 20, end=27)
resposta_q1 <- str_c(string1, string2, string3,string4, sep = " ")
str_to_title(resposta_q1)
```


Outra solução usando `str_split()`:
...
info <- url %>%
  ...
  str_to_title() %>% 
  unlist()
str_c(info[1], info[2], info[3], info[4], sep = " ")
```
...
```{r}
suspeitos <-c("Júlio","Alfredo","Antonela")
fez <- c("culpado","inocente","cúmplice")
motivo <- c("estava na cozinha e envenenou o café.","tem álibi.","tinha intenção de fugir com o assassino.")
str_c(suspeitos,"é",fez,"porque",motivo,sep = " ")
```
</details>
